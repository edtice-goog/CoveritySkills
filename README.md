...
| [coverity-pathout](coverity-pathout/README.md) | A function exceeded the path limit (`PATHOUT`, `--paths`), and you want to know which checker stopped, what it cost, and what may be hiding behind it | The functions and the checker that ran out of paths, the path-insensitive shape catalogue run over the idir and filtered to those functions, and the measured cost of the cut-off |
